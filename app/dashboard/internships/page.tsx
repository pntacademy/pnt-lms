    ...
    <div className="min-h-full font-sans text-slate-800 p-4 md:p-8">
      <header className="mb-8">
        <h1 className="text-3xl md:text-4xl font-black uppercase text-black tracking-tight flex items-center gap-3">
          <Briefcase size={36} className="text-[#dc0a2d]" strokeWidth={2.5} />
          Internships
        </h1>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        {internships.map((internship) => (
          <Card key={internship.id} className="border-4 border-black rounded-xl neo-shadow overflow-hidden bg-white flex flex-col group hover:-translate-y-1 transition-transform">
            <CardHeader className="border-b-4 border-black bg-slate-100 flex flex-row items-start justify-between pb-4">
              <div>
                <CardTitle className="text-xl font-black uppercase text-black leading-tight mb-2">
                  {internship.title}
                </CardTitle>
              ...
              {internship.status === "ACTIVE" ? (
                <Badge className="bg-green-100 text-green-700 border-2 border-green-700 font-black neo-shadow-sm hover:bg-green-100 uppercase ml-2 shrink-0">
                  Access Granted
                </Badge>
              ) : (
            ...
            <CardContent className="p-6">
               <p className="text-sm font-bold text-slate-600 mb-4">
                 <span className="text-black">Duration:</span> {internship.duration}
               </p>
            ...
            <CardFooter className="pt-0 p-6 mt-auto">
               <a 
                 ...
                 className={`w-full flex items-center justify-center gap-2 px-5 py-4 border-2 border-black rounded-lg font-black uppercase text-sm transition-all neo-shadow-sm ${
                   internship.status === "ACTIVE" 
                    ? "bg-[#ffcb05] text-black hover:bg-[#d4a017] hover:shadow-[4px_4px_0_rgba(0,0,0,1)] hover:-translate-y-1 active:translate-y-1 active:shadow-none" 
